  ...
  return (
    <main className="min-h-screen bg-[#FDFCFB] text-[#2D2E2E] selection:bg-indigo-100/50">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-[10%] -left-[10%] w-[40%] h-[40%] rounded-full bg-purple-50/50 blur-[120px]" />
        <div className="absolute top-[20%] -right-[5%] w-[30%] h-[30%] rounded-full bg-blue-50/40 blur-[100px]" />
        <div className="absolute -bottom-[10%] left-[20%] w-[35%] h-[35%] rounded-full bg-emerald-50/30 blur-[110px]" />
      </div>

      <div className="relative max-w-[1100px] mx-auto px-8 py-16 md:py-24">
        {/* Header */}
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
          ...
          {results && (
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white border border-slate-100 text-sm font-medium text-slate-600 hover:text-slate-900 hover:border-slate-200 hover:shadow-sm transition-all duration-300"
            >
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-1000 ease-out">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={idx} className={`${stat.bg} p-8 rounded-[2rem] border border-white shadow-[0_8px_30px_rgb(0,0,0,0.02)] backdrop-blur-sm transition-transform hover:-translate-y-1 duration-500`}>
                  <p className="text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-3">{stat.label}</p>
                  <p className={`text-4xl font-medium tracking-tighter ${stat.color}`}>{stat.value}</p>
                </div>
              ))}
            </div>

            {/* Reconciliation Comparison Table */}
            <div className="group bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.03)] overflow-hidden transition-all duration-700">
              <div className="px-10 py-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                <div>
                  <h3 className="text-base font-semibold text-slate-900">Comparative Reconciliation</h3>
                  <p className="text-xs text-slate-400 mt-1">Cross-referencing bank statements against internal ledgers</p>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold border-b border-slate-50">
                      <th className="pl-10 pr-6 py-6">Date</th>
                      <th className="px-6 py-6">Bank Description</th>
                      <th className="px-6 py-6 text-right">Amount</th>
                      <th className="px-6 py-6 text-center">Status</th>
                      <th className="pl-6 pr-10 py-6">Ledger Reference</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    ...
                    {results.matches.map((match: any, i: number) => (
                      <tr key={`match-${i}`} className="group/row hover:bg-slate-50/50 transition-all duration-300">
                        <td className="pl-10 pr-6 py-6 text-sm text-slate-400 font-light">
                        ...
                        <td className="px-6 py-6 text-sm font-semibold text-right text-slate-900">
                    ...
                    {results.unmatchedBank.map((tx: any, i: number) => (
                      <tr key={`umb-${i}`} className="bg-rose-50/10 group/row hover:bg-rose-50/20 transition-all duration-300">
                        <td className="pl-10 pr-6 py-6 text-sm text-slate-400 font-light italic">
                          {formatDate(tx.date)}
                        </td>
                        <td className="px-6 py-6">
                        ...
                        <td className="px-6 py-6 text-sm font-semibold text-right text-rose-600">
            ...
            <footer className="text-center pt-12 pb-20">
              <div className="inline-flex items-center gap-4">
                ...
                <p className="text-[10px] text-slate-300 font-bold uppercase tracking-[0.4em]">Designed for Operational Excellence</p>
                <div className="h-px w-8 bg-slate-100" />
              </div>
            </footer>
          </div>
